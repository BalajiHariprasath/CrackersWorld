...
function saveLoginInfo(username) {
    ...
    fetch(scriptURL, { method: 'POST', body: formData })
        .then(response => console.log('Login info saved!', response))
        .catch(error => console.error('Error saving login info!', error.message));
}

// Google Apps Script
function doPost(e) {
    const sheet = SpreadsheetApp.openById('1vbMWTCGZzBLmF4HcyQMbewVI5hwKpLwMIO6kEy0ZKB').getSheetByName('Sheet1');
    const cart = JSON.parse(e.parameter.cart);
    const customerDetails = JSON.parse(e.parameter.customerDetails);
    const loggedInUser = e.parameter.loggedInUser;

    cart.forEach(item => {
        sheet.appendRow([item.name, item.size, item.quantity, item.discountedPrice, item.discountedPrice * item.quantity]);
    });

    sheet.appendRow([]);
    sheet.appendRow(['Customer Details']);
    sheet.appendRow(['Name', customerDetails.name]);
    sheet.appendRow(['Mobile', customerDetails.mobile]);
    sheet.appendRow(['Address', customerDetails.address]);
    sheet.appendRow(['Logged In User', loggedInUser]);
